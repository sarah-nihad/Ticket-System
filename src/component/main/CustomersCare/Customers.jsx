...
class Customers extends React.Component {
  ...
  GetCustomers() {
    ...
    axios({
      ...
      .then(res => {
        ...
        for (let index = 0; index < res.data.data.length; index++) {
          let obj = {
            label: res.data.data[index].customer_name,
            value: res.data.data[index].customer_id,
          }
          arr.push(obj);
          idarr.push(res.data.data[index].customer_id);
        }
        this.setState({
          Allcustomer: arr, ids: idarr
        });
      })
      .catch(err => {
      });
  ...
  async getTasks() {
    ...
    await axios({
      url: Host + `tasks/tasks?params={"from_date":${this.state.date1},"to_date":${this.state.date2},"task_type_id":9}`,
      method: "GET",
      headers: headers,
                ...
                <div >
                  ...
                  <div id="apfot" ref={this.myRef}  >
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', width: '100%' }}>
                      ...
                      <NewCustomer onProfileDelete={() => this.componentDidMount()} users={this.state.users} key={1} onRefreshGeo={() => this.RefreshGeofences()}
                        type={this.state.type} priorities={this.state.priorities} Geofences={this.state.Geofences} onRefTask={() => this.getTasks()}
                        Allcustomer={this.state.Allcustomer} onGetCustomers={()=> this.GetCustomers()}   />
                      ...
                      <Row style={{ width: '100%', display: 'flex' }}   >
                        {this.state.dataF.map((item, i) => (
                          <Col md={6} key={i} id='noti_Get' style={{ marginTop: 40 }}    >
                            <CardCustomer name={item.task_title} time={item.dead_time} desc={item.description}
                              ...
                              customer={item.customer} Allcustomer={this.state.Allcustomer} />
